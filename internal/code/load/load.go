...
type (
	// A SchemaSpec holds a serializable version of an mongox.Schema
...
func (c *Config) Load() (*SchemaSpec, error) {
	spec, err := c.load()
	if err != nil {
		return nil, fmt.Errorf("mongox/load: parse schema dir: %w", err)
	}
	if len(c.Names) == 0 {
		return nil, fmt.Errorf("mongox/load: no schema found in: %s", c.Path)
	}
	var b bytes.Buffer
	...
	if err != nil {
		return nil, fmt.Errorf("mongox/load: execute template: %w", err)
	}
	buf, err := format.Source(b.Bytes())
	if err != nil {
		return nil, fmt.Errorf("mongox/load: format template: %w", err)
	}
	if err := os.MkdirAll(".mongox", os.ModePerm); err != nil {
		return nil, err
	}
	target := fmt.Sprintf(".mongox/%s.go", filename(spec.PkgPath))
	if err := os.WriteFile(target, buf, 0644); err != nil {
		return nil, fmt.Errorf("mongox/load: write file %s: %w", target, err)
	}
	defer os.RemoveAll(".mongox")
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if err != nil {
			return nil, fmt.Errorf("mongox/load: unmarshal schema %s: %w", line, err)
		}
...
// mongoxInterface holds the reflect.Type of mongox.Interface.
var mongoxInterface = reflect.TypeOf(struct{ mongox.Interface }{}).Field(0).Type

// load the mongox/schema info.
func (c *Config) load() (*SchemaSpec, error) {
	pkgs, err := packages.Load(&packages.Config{
		BuildFlags: c.BuildFlags,
		Mode:       packages.NeedName | packages.NeedTypes | packages.NeedTypesInfo | packages.NeedModule,
	}, c.Path, mongoxInterface.PkgPath())
	if err != nil {
	...
	mongoxPkg, pkg := pkgs[0], pkgs[1]
	if len(pkg.Errors) != 0 {
		return nil, c.loadError(pkg.Errors[0])
	}
	if len(mongoxPkg.Errors) != 0 {
		return nil, mongoxPkg.Errors[0]
	}
	if pkgs[0].PkgPath != mongoxInterface.PkgPath() {
		mongoxPkg, pkg = pkgs[1], pkgs[0]
	}
	var names []string
	iface := mongoxPkg.Types.Scope().Lookup(mongoxInterface.Name()).Type().Underlying().(*types.Interface)
		...
		for _, d := range f.Decls {
			...
			for _, s := range g.Specs {
				...
				astutil.Apply(st.Fields, func(c *astutil.Cursor) bool {
					...
					// Stop traversing the AST in case an ~mongox.Schema is embedded.
					return !embedSchema
				}, nil)
				if !embedSchema {
					locals[ts.Name.Name] = true
...
func filename(pkg string) string {
	...
	return fmt.Sprintf("mongox_%s_%d", name, time.Now().Unix())
}
...
func gorun(target string, buildFlags []string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mongox/load: %s", err)
	}
	return s, nil
}
